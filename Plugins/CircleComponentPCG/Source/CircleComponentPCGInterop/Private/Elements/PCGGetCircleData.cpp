...
{
		return LOCTEXT("GetCircleDataTooltip", "Builds analytic circle data from Circle Components on the selected actors.");
}
#endif
...
void FPCGGetCircleInteropElement::ProcessActor(FPCGContext* Context, const UPCGDataFromActorSettings* InSettings, AActor* FoundActor) const
{
	...
	for (UCircleComponent* CircleComponent : CircleComponents)
	{
		...
		UPCGCircleData* CircleData = PCGCircleHelpers::CreateCircleDataFromCircleComponent(Context, CircleComponent);
		if (!CircleData)
		{
			continue;
		}
