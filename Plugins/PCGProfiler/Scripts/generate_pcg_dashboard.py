...
def build_diagnosis(data, nodes):
    ...
    if pe < 60:
        out.append(f'并行效率偏低（{pe:.1f}）。')
    if hit is not None and hit < 0.4:
        out.append(f'缓存命中率偏低（{hit:.1%}）。')
    if ratio is not None and ratio > 2.0:
        out.append(f'点数膨胀明显（Out/In={ratio:.2f}）。')
    top = sorted(nodes, key=lambda n: n.get('total_ms', 0) or 0, reverse=True)[:3]
    if top:
        out.append('Top耗时节点: ' + ', '.join([f"{n.get('node_name','?')}({(n.get('total_ms') or 0):.1f}ms)" for n in top]))
    return out or ['未发现明显异常，建议关注跨run波动。']
...
def compare_runs(current, baseline):
    ...
    if run_delta > 10:
        result['regressions'].append(f'总时长回归 {run_delta:.2f}% (>10%)')
    if pe_delta < -8:
        result['regressions'].append(f'并行效率下降 {abs(pe_delta):.2f} 分 (>8)')
...
<h1>PCG Profiler 二期报告（Events 深挖）</h1><div id='app'></div>
<script>
...
<div class='panel'><h3>自动诊断</h3><ul>${{(payload.diagnosis||[]).map(x=>`<li>${{esc(x)}}</li>`).join('')}}</ul></div>
<div class='row'><div class='panel'><h3>稳定性 + 回归判定（stability/cross_run_node_stats/top_n）</h3><div id='stabilityBox'></div><div class='table-wrap'><table id='crossTbl'><thead><tr><th>Node</th><th>P50</th><th>P95</th><th>CV</th><th>Samples</th></tr></thead><tbody></tbody></table></div></div><div class='panel'><h3>TopN 节点（top_n）</h3><div class='small'>by_total_ms</div><div class='table-wrap'><table id='topTotalTbl'><thead><tr><th>Node</th><th>Total</th><th>P95</th><th>HitRate</th></tr></thead><tbody></tbody></table></div><div class='small'>by_peak_ms</div><div class='table-wrap'><table id='topPeakTbl'><thead><tr><th>Node</th><th>Max</th><th>Calls</th><th>Graph</th></tr></thead><tbody></tbody></table></div></div></div>
<div class='panel'><h3>缓存命中热力图（cache_analysis.node_cache_stats）</h3><div id='cacheHeat'></div></div>
<div class='panel'><h3>节点全量表 + 局部前后对比</h3><div class='controls'><input id='qNode' placeholder='搜索节点/图/组件'/><select id='graphNode'><option value=''>All Graph</option></select><select id='sortNode'><option value='total_ms'>total_ms</option><option value='p95_ms'>p95_ms</option><option value='cache_miss_count'>cache_miss_count</option><option value='estimated_memory_bytes'>est_mem</option></select></div><div class='row'><div class='table-wrap'><table id='nodeTbl'><thead><tr><th>Node</th><th>Graph</th><th>Total</th><th>P95</th><th>HitRate</th><th>Miss</th><th>Component</th></tr></thead><tbody></tbody></table></div><div id='cmpNode' class='panel'><div class='small'>点击节点查看对比</div></div></div></div>
<div class='panel'><h3>Events 时间线 + 明细</h3><div class='controls'><select id='phaseEv'><option value=''>All Phase</option></select><select id='threadEv'><option value=''>All Thread</option><option>GameThread</option><option>Worker</option><option>Unknown</option></select><select id='cacheEv'><option value=''>All Cache</option><option value='hit'>Hit</option><option value='miss'>Miss</option></select><input id='qEv' placeholder='搜索node/component/path'/></div><svg id='timeline'></svg><div class='table-wrap'><table id='eventTbl'><thead><tr><th>Start</th><th>Duration</th><th>Node</th><th>Phase</th><th>Thread</th><th>Cache</th><th>Component</th><th>Path</th></tr></thead><tbody></tbody></table></div></div>
<div class='row'><div class='panel'><h3>Drill-down（phase/thread/cache）</h3><div class='table-wrap'><table id='drillTbl'><thead><tr><th>Phase</th><th>Thread</th><th>Cache</th><th>Count</th><th>InclusiveTotal</th><th>QueueWaitTotal</th></tr></thead><tbody></tbody></table></div></div><div class='panel'><h3>线程负载图（timeline_windows）</h3><div id='winBox'></div></div></div>
<div class='panel'><h3>事件火焰图/调用树（events + parent_execution_path）</h3><div class='controls'><label><input type='checkbox' id='onlyCriticalCall'/> 仅显示关键路径节点</label></div><svg id='flame'></svg><div class='table-wrap'><table id='callTbl'><thead><tr><th>Depth</th><th>Node</th><th>Inclusive</th><th>Self</th><th>Count</th><th>Component</th><th>Path</th></tr></thead><tbody></tbody></table></div></div>
<div class='row'><div class='panel'><h3>关键路径甘特图（critical_path）</h3><svg id='gantt'></svg></div><div class='panel'><h3>生命周期与冗余链路</h3><div id='life'></div><div class='table-wrap'><table id='redTbl'><thead><tr><th>Parent</th><th>Child</th><th>Contrib</th><th>Ratio</th><th>Calls</th></tr></thead><tbody></tbody></table></div></div></div>
<div class='panel'><h3>导出</h3><ul><li><a href='nodes.csv'>nodes.csv</a></li><li><a href='events.csv'>events.csv</a></li><li><a href='compare.json'>compare.json</a></li><li><a href='summary.json'>summary.json</a></li><li><a href='dashboard.html'>dashboard.html</a></li></ul></div>`;
...
function fNodes(){{const q=(document.getElementById('qNode').value||'').toLowerCase();const g=document.getElementById('graphNode').value;const k=document.getElementById('sortNode').value;const arr=nodes.filter(n=>{{const t=`${{n.node_name||''}} ${{n.node_id||''}} ${{n.graph_name||''}} ${{n.component_name||''}}`.toLowerCase();if(q&&!t.includes(q))return false;if(g&&(n.graph_name||'')!==g)return false;return true;}});arr.sort((a,b)=>(b[k]||0)-(a[k]||0));return arr;}}
function renderNodes(){{const tb=document.querySelector('#nodeTbl tbody');tb.innerHTML='';for(const n of fNodes().slice(0,800)){{const tr=document.createElement('tr');tr.innerHTML=`<td>${{esc(n.node_name||'')}}</td><td>${{esc(n.graph_name||'')}}</td><td>${{fmt(n.total_ms||0)}}</td><td>${{fmt(n.p95_ms||0)}}</td><td>${{pct(n.cache_hit_rate||0)}}</td><td>${{fmt(n.cache_miss_count||0,0)}}</td><td>${{esc(n.component_name||'')}}</td>`;tr.addEventListener('click',()=>{{const m=Object.values(compare.node_component_diff||{{}}).find(v=>v.node_id===(n.node_id||'')&&(!n.component_name||v.component_name===(n.component_name||'')));const box=document.getElementById('cmpNode');if(!m){{box.innerHTML='<div class="small">未找到基线对比数据（需要baseline）</div>';return;}}const cls=(m.delta_inclusive_pct||0)>10?'bad':'good';box.innerHTML=`<h3>局部重建前后对比</h3><div><b>${{esc(m.node_name||n.node_name||'')}}</b></div><div class='small'>node_id=${{esc(m.node_id||'')}} | component=${{esc(m.component_name||'')}}</div><div>Current: <b>${{fmt(m.current_inclusive_total_ms||0)}}</b> ms</div><div>Baseline: <b>${{fmt(m.baseline_inclusive_total_ms||0)}}</b> ms</div><div>Delta: <b class='${{cls}}'>${{fmt(m.delta_inclusive_ms||0)}} ms (${{fmt(m.delta_inclusive_pct||0)}}%)</b></div><div>CacheHit current/baseline: ${{pct(m.current_cache_hit_rate||0)}} / ${{pct(m.baseline_cache_hit_rate||0)}}</div><div class='code'>${{esc(m.execution_path||'')}}</div>`;}});tb.appendChild(tr);}}}}
...
function renderEvents(){{const arr=fEvents().slice().sort((a,b)=>(a.first_seen_time_ms||0)-(b.first_seen_time_ms||0));const tb=document.querySelector('#eventTbl tbody');tb.innerHTML='';for(const e of arr.slice(0,1200)){{tb.insertAdjacentHTML('beforeend',`<tr><td>${{fmt(e.first_seen_time_ms||0)}}</td><td>${{fmt(Math.max(e.duration_ms||0,e.inclusive_ms||0))}}</td><td>${{esc(e.node_name||e.node_title||'')}}</td><td>${{esc(e.phase||'')}}</td><td>${{esc(e.thread_group||'Unknown')}}</td><td>${{e.cache_hit?'hit':'miss'}}</td><td>${{esc(e.component_name||'')}}</td><td title='${{esc(e.execution_path||'')}}'>${{esc((e.execution_path||'').slice(0,80))}}</td></tr>`);}}}}
...
function renderCmpTop(){{if(!compare.available)return;const s=compare.summary||{{}};const div=document.createElement('div');div.className='panel';div.innerHTML=`<h3>回归判定</h3><div>Run Duration Δ: <b class='${{(s.run_duration_delta_pct||0)>10?'bad':'good'}}'>${{fmt(s.run_duration_delta_pct||0)}}%</b></div><div>Parallel Efficiency Δ: <b class='${{(s.parallel_eff_delta||0)<-8?'bad':'good'}}'>${{fmt(s.parallel_eff_delta||0)}}</b></div><div>${{(compare.regressions||[]).length?(compare.regressions||[]).map(x=>`<div class='bad'>${{esc(x)}}</div>`).join(''):'<span class="good">未检测到一级回归</span>'}}</div>`;app.prepend(div);}}
function renderStability(){{const s=payload.stability_view||{{}};const box=document.getElementById('stabilityBox');box.innerHTML=`<div>run_duration_cv: <b>${{fmt(s.run_duration_cv||0,4)}}</b></div><div>parallel_efficiency_cv: <b>${{fmt(s.parallel_efficiency_cv||0,4)}}</b></div><div>top_node_p95_cv: <b>${{fmt(s.top_node_p95_cv||0,4)}}</b></div>`;const cross=s.cross_run_node_stats||[];const tb=document.querySelector('#crossTbl tbody');tb.innerHTML='';for(const r of cross.slice(0,120)){{tb.insertAdjacentHTML('beforeend',`<tr><td>${{esc(r.node_name||r.node_title||r.node_id||'')}}</td><td>${{fmt(r.p50_ms||0)}}</td><td>${{fmt(r.p95_ms||0)}}</td><td>${{fmt(r.cv||r.duration_cv||0,4)}}</td><td>${{fmt(r.sample_count||0,0)}}</td></tr>`);}}const tt=document.querySelector('#topTotalTbl tbody');tt.innerHTML='';for(const r of (s.top_n_by_total_ms||[]).slice(0,30)){{tt.insertAdjacentHTML('beforeend',`<tr><td>${{esc(r.node_name||r.node_title||r.node_id||'')}}</td><td>${{fmt(r.total_ms||0)}}</td><td>${{fmt(r.p95_ms||0)}}</td><td>${{pct(r.cache_hit_rate||0)}}</td></tr>`);}}const tp=document.querySelector('#topPeakTbl tbody');tp.innerHTML='';for(const r of (s.top_n_by_peak_ms||[]).slice(0,30)){{tp.insertAdjacentHTML('beforeend',`<tr><td>${{esc(r.node_name||r.node_title||r.node_id||'')}}</td><td>${{fmt(r.max_ms||r.peak_ms||0)}}</td><td>${{fmt(r.call_count||0,0)}}</td><td>${{esc(r.graph_name||'')}}</td></tr>`);}}}}
function renderCacheHeatmap(){{const rows=payload.cache_heatmap||[];const box=document.getElementById('cacheHeat');if(!rows.length){{box.innerHTML='<div class=\"small\">无 node_cache_stats 数据</div>';return;}}let h='';for(const r of rows){{const hit=Math.max(0,Math.min(1,r.hit_rate||0));const miss=(r.miss_count||0);const bad=hit<0.4?'bad':'';const w=(hit*100).toFixed(1);h+=`<div style='display:grid;grid-template-columns:320px 1fr 120px;gap:8px;align-items:center;margin:4px 0'><div title='${{esc(r.node_id||'')}}'>${{esc((r.node_name||r.node_id||'Unknown').slice(0,40))}}</div><div style='height:10px;background:#e5e7eb;border-radius:8px;overflow:hidden'><div style='height:10px;width:${{w}}%;background:${{hit<0.4?'#ef4444':hit<0.75?'#f59e0b':'#16a34a'}}'></div></div><div class='${{bad}}'>hit=${{(hit*100).toFixed(1)}}% miss=${{fmt(miss,0)}}</div></div>`;}}box.innerHTML=h;}}
...
renderCmpTop();renderStability();renderCacheHeatmap();renderNodes();renderEvents();renderTimeline();renderDrill();renderWins();renderCallAndFlame();renderGantt();renderLife();
</script></body></html>"""
...
def main():
    ...
    write_csv(out_dir / 'nodes.csv', nodes, [
        'node_id','node_name','graph_name','component_name','data_type','severity','total_ms','p95_ms','max_ms','avg_ms','call_count','game_thread_ratio','worker_thread_ratio','input_points_max','output_points_max','output_input_ratio_max','cache_hit_rate','cache_miss_count','estimated_memory_bytes','measured_memory_delta_sum_bytes'
    ])
